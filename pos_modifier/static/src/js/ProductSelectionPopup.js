odoo.define('point_of_sale.ProductSelectionPopup', function (require) {
    ...
    class ProductSelectionPopup extends AbstractAwaitablePopup {
        ...
        async _clickProduct(event) {
            ...
            const default_quantity = topping_data.default_quantity;
            ...
            if (item && item.length > 0) {
                ...
                if (item_products.length > 0 && multi_selection == false) {
                    Gui.showPopup("ErrorPopup", {
                        'title': _t("Wrong Selection"),
                        'body':  _t("You can only select one item"),
                    });
                    allow = false;
                    return
                } else {
                    ...
                    if (item_products.length > 0 && total_items >= topping_data.no_of_items) {
                        Gui.showPopup("ErrorPopup", {
                            'title': _t("Wrong Selection"),
                            'body':  _t("You can only select ") + topping_data.no_of_items + _t(" item from ") + topping_data.description,
                        });
                        return
                    }
                }
            ...
            for (var i = 0; i < order_menu.length; i++) {
                if (topping_data.id == order_menu[i].toppingId) {
                    ...
                    if (exist_product) {
                        exist_product['qty'] = exist_product.qty + default_quantity;
            ...
            if (allow) {
                order_menu.push({
                    ...
                    'include_price': topping_data.include_price,
                    'products': [{
                        'product_id': product.id,
                        'name_arabic': product.name_arabic,
                        'product_name': product.display_name,
        ...
        compute_all_compo(taxes, price_unit, quantity, currency_rounding, handle_price_include = true) {
            ...
            var total_excluded = round_pr(recompute_base(base, 0,0,0), initial_currency_rounding);
        ...
        async confirm() {
            ...
            if (this.env.order_menu.length == 0 && required_sub_item) {
                Gui.showPopup("ErrorPopup", {
                    'title': _t("Wrong Selection"),
                    'body':  _t("You can not confirm without select any item of min qty > 0 lines!"),
                });
                return
            } else {
                for (var i = 0; i < this.data.length; i++) {
                    if (this.data[i].no_of_min_items != 0) {
                        ...
                        if (this.data[i].no_of_min_items > min_items) {
                            Gui.showPopup("ErrorPopup", {
                                'title': _t("Wrong Selection"),
                                'body': _t("You Have to Select ") + this.data[i].no_of_min_items + _t(" item from ") + this.data[i].category,
                            });
                            return
                        }
                    }
            ...
            for (var i = 0; i < this.env.order_menu.length; i++) {
                ...
                if (total_items < topping_data.no_of_min_items && multi_selection != false) {
                    Gui.showPopup("ErrorPopup", {
                        'title': _t("Wrong Selection"),
                        'body': _t("You Must Have to Select ") + topping_data.no_of_min_items + _t(" item from ") + topping_data.category,
                    });
                    return
                }
                ...
                for (var j = 0; j < this.env.order_menu[i].products.length; j++) {
                    ...
                    own_data.push({
                        ...
                        'include_price': this.env.order_menu[i].include_price,
                    ...
                    if (this.env.order_menu[i].include_price) {
                        total_price += computed_price.total_excluded;
